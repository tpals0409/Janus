    ...
    <section className="task-development-surface">
      <div className="task-development-surface__toolbar">
        ...
        <Tabs
          ...
          className="h-full"
          labels={{
            terminal: <><TerminalIcon size={11} />터미널<kbd className="font-mono text-[10px] text-faint">⌘J</kbd></>,
            editor: <><Code2 size={11} />편집기<kbd className="font-mono text-[10px] text-faint">⌘P</kbd></>,
            preview: <><ExternalLink size={11} />미리보기<kbd className="font-mono text-[10px] text-faint">⌘⇧B</kbd></>,
          }}
        />
        <span className="ml-auto max-w-[280px] truncate font-mono text-[10px] text-faint">{task.workspace?.root_path}</span>
      </div>
            ...
            <div className="min-h-0 overflow-auto bg-panel p-3">
              {inspection ? (
                <div>
                  ...
                  <div className="mt-2 border border-border bg-base p-2 font-mono text-[10px] text-muted">
              ...
              ) : screenshot ? <img src={screenshot} alt="작업 미리보기 화면" className="w-full rounded border border-border" /> : <div className="grid h-full place-items-center text-center text-[10px] leading-relaxed text-faint">작업별로 격리된 브라우저 프로필에서 localhost 앱을 여세요.<br />콘솔, 네트워크, 화면 캡처, DOM/CSS 컨텍스트가 이 작업에 유지됩니다.</div>}
            </div>
          </div>
        </div>
      )}
