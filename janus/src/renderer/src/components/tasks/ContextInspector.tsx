      ...
      <div className="grid grid-cols-[220px_minmax(0,1fr)] gap-4">
        <aside className="border border-border-subtle bg-panel p-4">
          ...
          <div className="mt-4 flex items-end gap-1 font-mono">
            ...
            <span className="pb-1 text-[10px] text-faint">/ {limitTokens.toLocaleString()} token</span>
          </div>
          <div className="context-meter mt-3"><span style={{ width: `${Math.max(2, fill)}%` }} /></div>
          <dl className="mt-4 space-y-2 border-t border-border pt-3 text-[10px]">
        ...
        <section className="overflow-hidden border border-border-subtle bg-panel">
          <header className="flex items-center gap-2 border-b border-border px-4 py-3">
            ...
            <div><h4 className="text-[11px] font-semibold">모델에 전달되는 조각</h4><p className="mt-0.5 text-[10px] text-faint">출처·포함 이유·예상 비용을 세션 snapshot 기준으로 표시합니다.</p></div>
            <span className="ml-auto font-mono text-[10px] text-faint">{context.items.filter((item) => item.status === 'included').length}/{context.items.length} INCLUDED</span>
          </header>
            ...
            {context.items.map((item, index) => (
              <details key={item.id} className="group relative border-b border-border py-2.5 last:border-b-0">
                ...
                <summary className="flex cursor-pointer list-none items-center gap-3">
                  ...
                  <span className="min-w-0 flex-1"><strong className="block truncate text-[10.5px] font-medium text-fg">{item.label}</strong><span className="font-mono text-[10px] text-faint">{item.source}</span></span>
                  <Status tone="muted">{item.status === 'included' ? '포함' : '정책으로 제외'}</Status>
                  <span className="w-16 text-right font-mono text-[10px] text-muted">~{item.estimated_tokens.toLocaleString()} tok</span>
                </summary>
                <div className="ml-6 mt-2 border border-border bg-base px-3 py-2.5">
                  <pre className="max-h-36 overflow-auto whitespace-pre-wrap font-mono text-[10px] leading-relaxed text-muted">{item.content || '내용 없음'}</pre>
                  <div className="mt-2 flex items-center gap-1.5 border-t border-border pt-2 font-mono text-[10px] text-faint"><Database size={9} /> {item.chars.toLocaleString()}자 · {item.id}</div>
                </div>
              </details>
            ))}
          </div>
