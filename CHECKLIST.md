...
- [x] 검증 게이밍 차단: 실행 중 리비전이 바뀌면 통과로 기록하지 않는다 — 결과를 어느
      리비전에도 귀속할 수 없기 때문. error가 있는 run은 failed가 아니라 error로 접는다
      (`routers/verifications.py`, `domain.finish_verification_run`)
...
- [x] 부모 면제 해소 — 오케스트레이터의 write_file/edit_file도 워커와 같은 소유권
      테이블을 지난다 (`runtime._parent_write_guards`). 기본 파티션 `"*"`는 스키마
      설명이 이미 대안(owned_paths 선언)을 안내하므로 그대로 둔다
...
- [x] janus 롤만 8,000자 프롬프트 가드 면제 — 가장 큰 프롬프트가 유일하게 무상한
      (`runtime.py:136-139`)
- [x] 마지막 블록 하나가 임계를 넘으면 압축이 무력 — 블록 내 head/tail 절단 폴백
      (`agent.py:220`)
- [x] 페르소나가 인라인된 번들 파일 경로를 인용해 소형 모델의 존재하지 않는 경로
...
v1.3 에이전트 워크플로우 개선은 41/43 완료(#66·#67). 남은 미완료 20개는 전부
**코드로 닫을 수 없는 것**이다 — 사람이 앱을 띄워야 하는 수동 QA 12개, 외부 배포
결정까지 보류한 3개, 제품 결정이 필요한 2개, 그리고 아래 요약 3줄이다.

- [ ] **현재 P0: 실제 Electron 앱 전체 화면 및 30-A 채팅·오케스트레이션 수동 QA**
      — v1.3에서 바꾼 완료 게이트(acceptance command 실행)와 재시도 컨텍스트 주입이
      실제 27B 세션에서 어떻게 보이는지 여기서 확인한다. electron 44 메이저 업그레이드
      (#67) 후 첫 실행 확인도 포함한다.
- [ ] **제품 결정: 프로젝트당 Task 1개 직렬화를 풀 것인가** (v1.3 §35)
      — per-Task worktree 복원 또는 `ownership.py` 파티션 기반 병행. 현재 시스템
      최대의 구조적 병렬성 천장이다.
- [ ] **제품 결정: untrusted 스킬을 카탈로그 주입 전에 차단할 것인가** (v1.3 §38)
      — GitHub import UX가 바뀐다. `execution.context: fork`는 여전히 미구현이다.
