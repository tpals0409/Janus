...
## 2026-08-27 — P7: 재시작·비정상 종료 복구 QA

자동화 가능한 §30-A 복구 시나리오 3종을 실기기에서 통과 처리했다.

- **유휴 재시작(R1)** — 재시작 전후 세션 이벤트 85건·seq 완전 동일(복원 1회, 중복 0),
  세션 1개 유지, 재연결 후 후속 질문이 이전 답을 기억.
- **실행 중 재시작(R2)** — 워커 running 도중 앱 강제 종료 → 세션 idle·dispatch
  needs_you로 명시 복구, 워커 자발 재배치 0건, 워크스페이스 clean. 워커 성과 노트
  미주입은 terminal 전 사망이라 기록할 성과가 없던 경우로 설계 일치.
- **WS 비정상 종료(R3)** — close 핸드셰이크 없는 절단에도 서버가 턴을 정리해 세션
  idle, 중복 세션 없음, transcript 1회 복원, 후속 턴 정상.
- 이로써 §30-A의 자동화 가능 항목은 전부 소진. 남은 것: queue 상한·유실 2건
  (슬롯 1 강제 필요, 재현 곤란으로 보류), IME·화면 상태·변경 패널 UI 등 수동 항목.

### 후속: 모델 생성 슬롯 기본값 5 → 3 (같은 날)

- PR #9 때 "3 vs 5 미정"으로 5가 출하됐던 값을 분석 권고치 3으로 되돌렸다.
  근거: 48GB 머신에서 KV 캐시는 스트림별(워커 예산 49K = 최악 5~12GB/스트림)이라
  5스트림 최악 케이스는 스왑행이고, Metal 1개라 동시 디코딩의 실이득(도구 I/O
  겹침)은 2~3에서 포화하며, MTP 승인율은 동시성에 반비례한다. 실측으로도 이번
  사이클 전체에서 최대 동시 생성 3·슬롯 대기 p95 0.012ms — 5가 필요한 적이 없었다.
...
## 2026-08-27 — v1.0.16 릴리스

- 사용자 실기기 QA 라운드의 수정·개선 3건을 담은 릴리스: Task를 다시 열면 재개
...
## 2026-08-27 — 세션 수명을 화면에서 분리 (실기기 QA 지시)

- "앱 내의 어떤 페이지도 세션을 멈추게 하면 안 된다" — 세션 런타임(소켓·이벤트·
  승인·턴 상태)을 스토어 상태가 아닌 모듈 레지스트리(liveRuntimes, Task당 1개)로
  옮겼다. 화면 전환·사이드바 클릭·프로젝트 이동은 뷰 상태만 갈아끼우고 소켓은
  계속 살아 이벤트를 쌓는다.
- 같은 Task 재선택(사이드바 클릭이 정확히 이 경로였다)은 세션을 파괴하지 않고
  재부착만 한다. 실행 중이던 Task로 돌아오면 자리 비운 사이의 라이브 이벤트·승인
  요청·턴 상태까지 그대로 복원된다(유휴면 영속 기록을 진실로 재동기화).
- 소켓을 닫는 것은 명시적 행위뿐이다: 새 시도(런타임 교체), 세션 중지, 프로젝트
  제거(closeAllLiveRuntimes).
- 테스트: 자동 재연결 2건 갱신 + "이동 후 복귀에도 소켓 생존·재부착" 1건 추가
  (`store.autoconnect.test.ts`), 렌더러 30건·tsc 통과.

## 2026-08-27 — v1.0.18 릴리스

- 실기기 QA 3라운드분: 세션 수명을 화면에서 분리(41dc635 — 어떤 페이지 이동도
  세션을 멈추지 않는다, Task당 라이브 런타임 레지스트리), 파일 트리 Git 상태
  배지(d39736c — 변경 파일 M/A/U 색·배지, 변경 포함 폴더 점, 헤더 카운트).

## 2026-08-27 — v1.0.19 릴리스

- 승인 반복 피로 해소(d3d0c6e): 기억 가능한 승인 요청에서 세션 범위 허용("이
  세션에서 명령/파일 수정 허용")을 primary 버튼으로, "이번만 허용"을 quiet으로.

## 2026-08-27 — 승인 기억을 세션 → 작업 단위로

- 실기기 QA 재보고: 세션 범위 허용을 눌러도 새 시도(새 세션)마다 다시 물었다.
  DB 확인 결과 같은 워크스페이스에서 세션 4개가 셸·쓰기 허용을 각각 다시 받았다 —
  기억 키가 session_id라서 시도마다 리셋되는 구조였다.
- 승인 기억을 작업(워크스페이스) 단위로 확장: WS 접속 시 허용 집합을
  `list_workspace_approval_scopes`로 구성해 이전 시도의 허용이 유효하고, 회수는
  `revoke_workspace_approval_scope`로 전 세션에 미친다. 부여 기록은 세션 id로
  남겨 출처를 보존한다. UI 문구도 "이 작업에서 … 허용 — 새 시도에도 유지"로 정정.
- 테스트: 작업 단위 조회·새 세션 관점·전역 회수(`test_domain.py` 확장),
  Python 245건·렌더러 31건·tsc 통과.

## 2026-08-27 — v1.0.20 릴리스

- 승인 기억 작업 단위 확장(a12ba5a)을 담은 패치 릴리스: 한 번 허용하면 같은 Task의
  새 시도에서도 유지되고, 회수는 전 세션에 미친다.

## 2026-08-27 — 설정 다이얼로그 (모델 런타임 손잡이)

- 타이틀바 우측 기어 버튼 → 설정 다이얼로그: MTP 정책(필수/선호/끔), 모델 동시
  생성 슬롯(1~8, vram_sizing 게이트 안내 포함), 프롬프트 캐시(APC) 토글.
- 값은 userData의 runtime-settings.json에 영속하고, 우선순위는 환경변수 > 설정
  파일 > 기본값 — env로 고정된 필드는 비활성화되고 사유가 표시된다.
- 저장 시 영향을 받는 서비스만 재시작한다(MTP·APC → 모델 서버, 슬롯 → 백엔드).
...
## 2026-08-27 — v1.0.21 릴리스

- UI 라운드 + 구독형 모델을 묶은 릴리스: 사이드바 접기(⌘B)·타이틀바 브랜드 제거·
  설정 다이얼로그(런타임 손잡이 + 모델 프로필 전환)·설정 버튼 하단 좌측 이동,
  구독형 CLI 프로바이더(Claude Code·Codex, 스키마 v26)와 사전 인증 게이트,
  마이그레이션 FK-off 러너 수정.

### 후속: 설정을 모델 종류에 맞게 분리 (같은 날)

- 지적: 구독형 모델을 선택해도 로컬 전용 손잡이(MTP·슬롯·APC)가 그대로 보였고,
  더 큰 손해는 구독형 위주로 써도 로컬 27B(16GB)+MTP가 계속 떠 있는 것.
- 런타임 섹션에 "로컬 모델 서버" 마스터 토글 신설 — 끄면 supervisor가 MLX를
  스폰하지 않고 종료하며(phase 'disabled'), 상태바·타이틀바는 "로컬 모델 꺼짐"으로
  정직하게 표시(로딩 펄스 없음). 로컬 전용 손잡이는 서버 꺼짐 시 비활성화.
- 구독형 프로필 선택 시 "이 섹션의 영향을 받지 않음 + 서버 꺼서 16GB 절약" 안내.
- 자동 시작 게이트를 프로바이더 인지형으로: 구독형 프로필은 mlx 준비를 기다리지
  않는다 — 로컬 서버를 꺼도 구독형 대화가 즉시 시작된다.
- 테스트: 구독형 안내·토글 재시작 경고·손잡이 비활성(SettingsPage), localServer
  클램프(runtime-settings), 전체 36건·tsc 통과.

## 2026-08-27 — v1.0.22 릴리스
